# -*- coding: utf-8 -*-
# memory_heads.py - ИСПРАВЛЕННАЯ ВЕРСИЯ v11.0 (MIMO + DataManager fix)
import threading
import torch
...
class ThaliaMambaHead(nn.Module):
    ...
    def _process_chunk_mimo(self, x_chunk, dt_chunk, B_chunk, C_chunk, A, current_hidden):
        """
        🔥 ИСПРАВЛЕНО: Линейная рекуррентность без квадратичного взрыва памяти O(L^2).
        Работает быстрее для коротких чанков (L=32) и экономит сотни мегабайт VRAM.
        """
        batch_size, chunk_len, inner_dim = x_chunk.shape
        
        outputs = []
        h = current_hidden  # [B, inner_dim, d_state]
        
        # Предрасчет A для векторизации
        A_broadcast = A.unsqueeze(0) # [1, inner_dim, d_state]

        # Линейный скан по длине чанка
        for t in range(chunk_len):
            x_t = x_chunk[:, t, :]    # [B, inner_dim]
            dt_t = dt_chunk[:, t, :]  # [B, inner_dim]
            B_t = B_chunk[:, t, :]    # [B, d_state]
            C_t = C_chunk[:, t, :]    # [B, d_state]

            # Дельта A: [B, inner_dim, d_state]
            dA_t = torch.exp(dt_t.unsqueeze(-1) * A_broadcast)
            
            # Дельта B (с учетом dt_t): [B, inner_dim, d_state]
            dB_t = dt_t.unsqueeze(-1) * B_t.unsqueeze(1)
            
            # Обновление скрытого состояния (рекуррентность)
            h = dA_t * h + dB_t * x_t.unsqueeze(-1)
            
            # Выходной сигнал: [B, inner_dim]
            y_t = (h * C_t.unsqueeze(1)).sum(dim=-1) + self.D * x_t
            outputs.append(y_t)

        # Собираем выходы обратно в чанк
        y_out = torch.stack(outputs, dim=1)  # [B, chunk_len, inner_dim]
        
        # Защита от NaN
        y_out = torch.nan_to_num(y_out, nan=0.0, posinf=1.0, neginf=-1.0)
    ...
    def forward(self, x, hidden_state=None, training_mode=False, autoencoder_mode=False):
        ...
        for i in range(0, seq_len, self.chunk_size):
            ...
            # 🔥 Проверка размерностей перед вызовом
            # x_chunk: [B, chunk_len, inner_dim]
            # dt_chunk: [B, chunk_len, inner_dim]
            # B_chunk: [B, chunk_len, d_state]
            # C_chunk: [B, chunk_len, d_state]
            # current_hidden: [B, inner_dim, d_state]
            
            chunk_out, current_hidden = self._process_chunk_mimo(
...
class StabilizedContrastiveLoss(nn.Module):
    ...
    def forward(self, good_output, good_target, bad_output, bad_target, include_reconstruction=True):
        ...
        if mean_diff > self._current_margin * 1.1:  # Превышаем margin на 10%+
            # Медленно растем
            self._current_margin = min(self.max_diff, self._current_margin * self.growth)
            self._stuck_counter = 0
        elif mean_diff < self._current_margin * 0.8:  # Ниже 80% от margin
            # 🔥 БЫСТРЕЕ падаем при неудаче
            self._current_margin = max(self.base_margin, self._current_margin * 0.97)  # Было 0.995
        ...
        target_margin = self._current_margin
...
class MetaCognitivePredictor(nn.Module):
    ...
    def _check_similar_to_errors_weighted(self, current_state):
        # (оставлено как было — работает стабильно)
        batch_size = current_state.shape[0]
        ...
        for b in range(batch_size):
            ...
            if ptr == 0:
                continue
            errors = self._error_memory[b, :ptr]
            if errors.shape[0] == 0:
                continue
            weights = self._recency_weights[:ptr].to(current_state.device)
            weights = weights / weights.sum()
            sims = F.cosine_similarity(current_state[b:b+1], errors, dim=-1)
    ...
    def _check_similar_to_success_weighted(self, current_state):
        ...
        for b in range(batch_size):
            ...
            weights = self._recency_weights[:ptr].to(current_state.device)
            weights = weights / weights.sum()
            sims = F.cosine_similarity(current_state[b:b+1], successes, dim=-1)
            similarities[b] = (sims * weights).sum()
        return similarities
    ...
    def get_stats(self, batch_idx=0):
        ...
        return {
            ...
            "novelty_exposure": float(self._novelty_exposure[batch_idx].item())
        }
    ...
    def get_state(self):
        ...
        buffer_names = [
            ...
            '_recent_error_rate', '_recent_success_rate', '_novelty_exposure', '_learning_momentum',
...
# ===================================================================
# ADAPTIVE MEMORY HEADS (С ИНТЕЛЛЕКТУАЛЬНЫМИ HARD NEGATIVES) - ВЕРСИЯ 11.0
# ===================================================================
class AdaptiveMemoryHeads(nn.Module):
    """Комбинированный модуль памяти (Mamba/RWKV/Transfomer) - v11.0 с MIMO-оптимизацией"""
    ...
    def __init__(self, config):
        ...
        logger.info(f"📝 Mamba-система v11.0 с MIMO-оптимизацией (chunk_size={chunk_size})")
...
# ===================================================================
    ...
    def _check_and_trigger_sleep_async(self):
        """🔥 Запускает сон в фоновом потоке с ПОЛНОЙ ИЗОЛЯЦИЕЙ весов"""
        try:
            ...
            if not self._check_and_set_sleep_flags():
                return False
            ...
            logger.info(f"💤 Запуск сна НА КОПИЯХ: {thoughts_count}/{notebook_size} ({status})")
            
            dream_thread = threading.Thread(
                target=self._background_dream_wrapper, 
                daemon=True,
                name=f"ThaliaDream_{self.sleep_cycles}"
            )
            dream_thread.start()
    ...
    def _internal_dream_phase_with_copies(self, writer_copy, reader_copy, thoughts):
        ...
        try:
            ...
            # ===========================================================
            # 🔥🔥🔥 ЭТАП 1: Обучение на копиях (ГРАДИЕНТЫ ЕСТЬ)
            # ===========================================================
            ...
            if hasattr(self, 'centroid_memory'):
                try:
                    ...
                    # 🔥🔥🔥 КРИТИЧЕСКИ ВАЖНО: блокируем доступ к центроидной памяти
                    with self._model_lock:
                        if hasattr(self.centroid_memory, '_ensure_device_consistency'):
                            self.centroid_memory._ensure_device_consistency()
    ...
    def _perform_contrastive_sleep_on_copies(self, positive_thoughts, hard_negative_thoughts, 
        ...
        contrastive_loss = StabilizedContrastiveLoss(
            base_margin=0.6,
            alpha=0.5,
            max_diff=1.0,
            growth=1.05,
    ...
    def _process_mamba_writer(self, candidates, context_vectors, mood_influence=0.0):
        """🔥 ОБНОВЛЁННЫЙ Writer с учетом влияния настроения и гарантированным сохранением surprise"""
        try:
            model_device = next(self.parameters()).device
            ...
            if context_vectors is not None and context_vectors.shape[1] > 0:
                ...
                if enriched_candidates:
                    enriched_stack = torch.stack(enriched_candidates)
                    if enriched_stack.dim() == 3:
                        enriched_stack = enriched_stack.transpose(0, 1)
                    elif enriched_stack.dim() == 2:
                        enriched_stack = enriched_stack.unsqueeze(0)
                    
                    # Конкатенируем с основным входом Mamba
                    mamba_input = torch.cat([mamba_input, enriched_stack], dim=1)
                    #logger.info(f"🧬 Добавлено {len(enriched_candidates)} ANTI-MUSH кандидатов")
    ...
    def _report_status_with_gpu_monitoring(self):
        try:
            ...
            logger.info(f"\n🧠 MAMBA v11.0 [шаг {self.step_count}]")
...
# ===================================================================
    def _generate_hard_negative(self, positive_thought, preferred_method=None):
        ...
        try:
            ...
            negative_thought = positive_thought.copy()
    ...
    def _deduplicate_notebook(self, thoughts, similarity_threshold=0.88):
        ...
        if duplicates_to_remove:
            ...
            thoughts_to_remove = {id(positive_thoughts[valid_indices[i]]) for i in duplicates_to_remove}
            cleaned_thoughts = [t for t in thoughts if id(t) not in thoughts_to_remove]
...
# ===================================================================
# 🗄️ АРХИВНЫЕ МЕТОДЫ (НЕ ИСПОЛЬЗУЮТСЯ - сохранены для истории)
# ===================================================================
    def _internal_dream_phase(self):
        """
        🗄️ АРХИВНЫЙ МЕТОД - НЕ ИСПОЛЬЗОВАТЬ!
        Используйте _internal_dream_phase_with_copies вместо этого.
        """
        logger.error("❌ ПОПЫТКА ВЫЗОВА УСТАРЕВШЕГО МЕТОДА _internal_dream_phase!")
        import traceback
        traceback.print_stack()
        raise RuntimeError("DEPRECATED METHOD - используйте _internal_dream_phase_with_copies")

    def _perform_contrastive_sleep(self, positive_thoughts, hard_negative_thoughts, gaslight_thoughts, device):
        """
        🗄️ АРХИВНЫЙ МЕТОД - НЕ ИСПОЛЬЗОВАТЬ!
        Используйте _perform_contrastive_sleep_on_copies вместо этого.
        """
        logger.error("❌ ПОПЫТКА ВЫЗОВА УСТАРЕВШЕГО МЕТОДА _perform_contrastive_sleep!")
        import traceback
        traceback.print_stack()
        raise RuntimeError("DEPRECATED METHOD - используйте _perform_contrastive_sleep_on_copies")
...
# ===================================================================    
    ...
    def load_checkpoint(self, checkpoint_path, optimizer=None, scheduler=None):
        ...
        if 'mamba_writer_state' in checkpoint and checkpoint['mamba_writer_state'] is not None:
            self.mamba_writer_state = checkpoint['mamba_writer_state'].to(self.config.device)
        if 'mamba_reader_state' in checkpoint and checkpoint['mamba_reader_state'] is not None:
            self.mamba_reader_state = checkpoint['mamba_reader_state'].to(self.config.device)
...
if __name__ == "__main__":
    logger.info("🧠 memory_heads.py v11.0 с MIMO-оптимизацией и DataManager fix загружен успешно")